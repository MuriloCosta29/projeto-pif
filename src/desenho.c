...
void desenhar_gol_bonito(float x, float y, float largura, float altura) {
    ...
    for (int i = 1; i < 8; i++) {
        ...
        Vector2 p2 = {
            fundoSupEsq.x + (fundoSupDir.x - fundoSupEsq.x) * t,
            fundoSupEsq.y
        };

        Vector2 p3 = {
            frenteInfEsq.x + (frenteInfDir.x - frenteInfEsq.x) * t,
            frenteInfEsq.y
        };

        Vector2 p4 = {
            fundoInfEsq.x + (fundoInfDir.x - fundoInfEsq.x) * t,
            fundoInfEsq.y
        };
    ...
    for (int i = 1; i < 6; i++) {
        ...
        Vector2 esqFrente = {
            frenteSupEsq.x,
            frenteSupEsq.y + altura * t
        };

        Vector2 dirFrente = {
            frenteSupDir.x,
            frenteSupDir.y + altura * t
        };

        Vector2 esqFundo = {
            fundoSupEsq.x,
            fundoSupEsq.y + altura * t
        };

        Vector2 dirFundo = {
            fundoSupDir.x,
            fundoSupDir.y + altura * t
        };
...
void desenhar_medidor(Jogo *jogo) {
    ...
    DrawRectangle(
        ...
        jogo->medidor_atual.y,
        jogo->medidor_atual.largura,
        jogo->medidor_atual.altura,
        LIGHTGRAY
    );
    ...
    float marcadorX = jogo->medidor_atual.x +
        jogo->medidor_atual.valor_atual * jogo->medidor_atual.largura;
...
void desenhar_hud(Jogo *jogo) {
    ...
    DrawText(TextFormat("Chances: %d", jogo->chances_restantes), 20, 80, 20, BLACK);

    DrawText(TextFormat("Direcao: %.2f", jogo->direcao_chute), 20, 120, 18, BLACK);
